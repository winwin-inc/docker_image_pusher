...
class TestDataFile:
    ...
    def test_extract_tags_from_data_file(self, mock_data_file: Path):
        ...
        for expected_tag in expected_tags:
            assert any(expected_tag in tag for tag in tags), f"Expected tag {expected_tag} not found"
    ...
    def test_parse_sample_images(self, sample_images):
        """测试解析示例镜像"""
        from src.image.parser import parse_image
    ...
    def test_specific_tag_patterns(self, sample_tags):
        ...
        patterns = {
            ...
            'datahub': [f'datahub-{name}-v1.2.0.1' for name in [
                'actions', 'elasticsearch-setup', 'frontend-react',
                'gms', 'ingestion', 'kafka-setup', 'mysql-setup',
                'postgres-setup', 'upgrade'
            ]],
            'victoria': [f'victoria-{metric}-v1.113.0' for metric in [
                'logs', 'metrics'
            ]]
        }
